/**
 * Session Aliases Library for CodeBuddy
...
function saveAliases(aliases) {
  ...
  try {
    ...
    // On Unix/macOS, rename(2) atomically replaces the destination 鈥?skip the
...
function renameAlias(oldAlias, newAlias) {
  ...
  saveAliases(data);
  return { success: false, error: 'Failed to save renamed alias 鈥?rolled back to original' };
}
