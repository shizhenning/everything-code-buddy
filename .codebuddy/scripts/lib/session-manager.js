/**
 * Session Manager Library for CodeBuddy
 * Provides core session CRUD operations for listing, loading, and managing sessions
 *
 * Sessions are stored as markdown files in ~/.codebuddy/sessions/ with format:
...
function parseSessionFilename(filename) {
  ...
  // Reject impossible dates like Feb 31, Apr 31 鈥?Date constructor rolls
  // over invalid days (e.g., Feb 31 鈫?Mar 3), so check month roundtrips
...
function getAllSessions(options = {}) {
  ...
  // Note: cannot use `|| default` because 0 is falsy 鈥?use isNaN instead.
