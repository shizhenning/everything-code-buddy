...
async function main() {
  ...
  const sessionId = process.env.CODEBUDDY_SESSION_ID || 'default';
  const counterFile = path.join(getTempDir(), `codebuddy-tool-count-${sessionId}`);
  ...
  try {
    ...
    try {
      ...
      if (bytesRead > 0) {
        ...
        // Clamp to reasonable range 鈥?corrupted files could contain huge values
