...
# ‚ù?WRONG: Overly broad permissions
iam_role:
  permissions:
    - s3:*  # All S3 actions
  resources:
...
```typescript
// ‚ú?CORRECT: Use cloud secrets manager
...
// ‚ù?WRONG: Hardcoded or in environment variables only
...
```terraform
# ‚ú?CORRECT: Restricted security group
...
```typescript
// ‚ú?CORRECT: Comprehensive logging
...
```yaml
# ‚ú?CORRECT: Secure GitHub Actions workflow
name: Deploy
...
```typescript
// ‚ú?CORRECT: Cloudflare Workers with security headers
export default {
...
```terraform
# ‚ú?CORRECT: Automated RDS backups
...
```bash
# ‚ù?WRONG: Public bucket
aws s3api put-bucket-acl --bucket my-bucket --acl public-read

# ‚ú?CORRECT: Private bucket with specific access
