...
Options:
  - "User-level (~/.codebuddy/)" â€?"Applies to all your Claude Code projects"
  - "Project-level (.codebuddy/)" â€?"Applies only to the current project"
...
Options:
  - "Framework & Language" â€?"Django, Spring Boot, Go, Python, Java, Frontend, Backend patterns"
...
- If a skill references `~/.codebuddy/settings.json` â€?this is usually fine (settings are always user-level)
- If a skill references `~/.codebuddy/skills/` or `~/.codebuddy/rules/` â€?this may be broken if installed only at project level
...
- For `continuous-learning-v2`, the `~/.codebuddy/homunculus/` directory is always user-level â€?this is expected and not an error.
