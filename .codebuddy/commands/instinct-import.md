...
Or if `CODEBUDDY_PLUGIN_ROOT` is not set (manual installation):
...
## New Instincts (8)
These will be added:
  ‚ú?use-zod-validation (confidence: 0.7)
  ‚ú?prefer-named-exports (confidence: 0.65)
  ‚ú?test-async-functions (confidence: 0.8)
  ...
...
Already have similar instincts:
  ‚ö†Ô∏è prefer-functional-style
     ...
     ‚Ü?Keep local (higher confidence)

  ‚ö†Ô∏è test-first-workflow
     Local: 0.75 confidence
     Import: 0.9 confidence
     ‚Ü?Update to import (higher confidence)

## Conflicting Instincts (1)
These contradict local instincts:
  ‚ù?use-classes-for-services
     Conflicts with: avoid-classes
     ‚Ü?Skip (requires manual resolution)
