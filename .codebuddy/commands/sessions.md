...
Manage Claude Code session history - list, load, alias, and edit sessions stored in `~/.codebuddy/sessions/`.
...
```bash
node -e "
const sm = require((process.env.CODEBUDDY_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.claude'))+'/scripts/lib/session-manager');
const aa = require((process.env.CODEBUDDY_PLUGIN_ROOT||require('path').join(require('os').homedir(),'.claude'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.codebuddy/sessions/' + session.filename);
console.log('');
...
- Sessions are stored as markdown files in `~/.codebuddy/sessions/`
- Aliases are stored in `~/.codebuddy/session-aliases.json`
